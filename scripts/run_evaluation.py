...
def _train_all(pre, split, eval_key, config, include_gbs):
    ...
    cox = fit_cox(fr_tr)
    preds["Cox"] = (cox_risk(cox, fr_ev), cox_survival(cox, fr_ev), cox_risk(cox, fr_tr))
    cox_model, cox_names = cox, names

    # Cox + splines
    xtr_s, xev_s, _, names_s = build_spline_features(X["train"], X[eval_key], X["train"], names)
    ...
    preds["Cox+splines"] = (cox_risk(cox_s, sfr_ev), cox_survival(cox_s, sfr_ev), cox_risk(cox_s, sfr_tr))

    # RSF
    y_tr = to_structured(dur["train"], ev["train"])
    rsf = fit_rsf(X["train"], y_tr)
    preds["RSF"] = (rsf_risk(rsf, X[eval_key]), rsf_survival(rsf, X[eval_key]), rsf_risk(rsf, X["train"]))

    # GBS (só se couber — ADR-010)
    if include_gbs:
        gbs = fit_gbs(X["train"], y_tr)
        preds["GBS"] = (gbs_risk(gbs, X[eval_key]), gbs_survival(gbs, X[eval_key]), gbs_risk(gbs, X["train"]))

    # DeepSurv
    set_global_seed(1)
    yds_tr, yds_va = deepsurv.make_target(dur["train"], ev["train"]), deepsurv.make_target(dur["val"], ev["val"])
    ...
    # DeepHit
    set_global_seed(1)
    lt = deephit.fit_label_transform(deephit.build_label_transform(), dur["train"], ev["train"])
    ydh_tr, ydh_va = deephit.make_target(lt, dur["train"], ev["train"]), deephit.make_target(lt, dur["val"], ev["val"])
...
def main() -> int:
    ...
    parser.add_argument("--no-gbs", action="store_true", help="pular o GBS (não escala; ADR-010)")
    ...
    config = TrainConfig()
    config.epochs = 30 if not args.full else config.epochs
    eval_df = getattr(split, args.eval_set)
    dur_e, ev_e, era_e = eval_df["duration"].to_numpy(), eval_df["event"].to_numpy(), eval_df[ERA_COLUMN].to_numpy()
    ...
    if args.full and include_gbs:
        print("[ADR-010] --full: GBS pulado (O(n²), inviável em ~944k; ele entra só na amostra fixa).")
        include_gbs = False

    print(f"\ntreino {len(split.train):,} | avaliação ({args.eval_set}) {len(eval_df):,} | n_boot {args.n_boot} | GBS={include_gbs}")
    preds_raw, (cox, cox_names) = _train_all(pre, split, args.eval_set, config, include_gbs=include_gbs)

    # --- 1. Tabela comparativa ---
    preds = [ModelPrediction(name, r[0], r[1]) for name, r in preds_raw.items()]
    table = comparison_table(preds, dur_e, ev_e, y_train, tau, n_boot=args.n_boot)
    ...
    print(calibration_by_decile(surv_top, dur_e, ev_e, CALIBRATION_HORIZON).to_string(index=False, float_format=lambda x: f"{x:.4f}"))

    # --- 3. Métricas por era ---
    print("\n=== Métricas estratificadas por era ===")
    print(metrics_by_era(era_e, dur_e, ev_e, risk_top, surv_top, y_train, tau).to_string(index=False, float_format=lambda x: f"{x:.4f}"))
    ...
    print(f"  n_train={tv['n_train']:,} n_eval={tv['n_eval']:,} | Harrell temporal = {tv['harrell_temporal']:.4f}")
    ...
    x_unknown = pre.transformer.transform(unk[INPUT_COLUMNS]).astype(np.float32)
    fr_unknown = build_cox_frame(x_unknown, cox_names, np.zeros(len(x_unknown)), np.ones(len(x_unknown)))
